...
In @sec-pdf-rag, we implement a multi-document PDF RAG system.

::: callout-note
**[llama-index](https://docs.llamaindex.ai/en/stable/):** A powerful framework for building LLM applications.
The package is however evolving rapidly and functionalities are subject to change.

**pdf-rag GitHub Repo & HF Space PDF Metadata Demo:** Check out the code and a demo. 

- [pdf-rag GitHub Repo](https://github.com/nbrosse/pdf-rag) 
- [HF Space Demo for PDF Metadata](https://huggingface.co/spaces/nicolasb92/pdf-rag-metadata-demo)
:::
...
Here are the parsed structures for the portrait and landscape documents examples shown above, @lst-landscape-structure-parsed for @lst-landscape-structure-ex and @lst-portrait-structure-parsed for @lst-portrait-structure-ex.
You can also find them on the huggingface space demo for metadata and structure [HF Space Demo for PDF Metadata](https://huggingface.co/spaces/nicolasb92/pdf-rag-metadata-demo).
...
Using these parsers, we get document nodes that we can incorporate into a hierarchical index. The `TreeIndex` class (in `src/pdf_rag/tree_index.py`) extends `BaseIndex` to provide a hierarchical tree-based indexing structure for document nodes. It's specifically designed to handle both portrait (reports) and landscape (presentations) formatted documents.
...
The PDF RAG (PDF Retrieval and Generation) module is designed to facilitate document retrieval and generation tasks using Vision Language Models (VLMs) and advanced indexing techniques. It leverages the metadata and structure extracted from PDF documents to enable efficient search, summarization, and content generation.

[Multi-Document Agents llama-index notebook](https://docs.llamaindex.ai/en/stable/examples/agent/multi_document_agents-v1/)