...
description: "Convolving a singular measure with a Gaussian of width σ produces a smooth density whose logarithmic derivatives blow up as σ → 0. Zooming in at scale σ replaces the support by its inward tangent cone, and every leading coefficient becomes a derivative of the log-Gaussian-mass of that cone — a manifold-valued inverse Mills ratio."
lightbox: true
citation:
...
This post is about a paper written with Arnak Dalalyan, [*Boundary-layer asymptotics for Gaussian-smoothed singular measures*](https://arxiv.org/abs/2607.04514) [@brosse2026boundary], which answers three questions at once: at what rate do the score $\nabla\log p_\sigma$, the log-Hessian $\nabla^2\log p_\sigma$, and the scale derivative $\partial_\sigma\nabla\log p_\sigma$ blow up; what the leading coefficients are; and which features of the geometry of the support they remember.

The short answer is a single sentence. **Zoom in at scale $\sigma$: the support is replaced by its inward tangent cone, and every leading coefficient is a derivative of the logarithm of the Gaussian mass of that cone.** In one dimension, at the endpoint of an interval, the Gaussian mass of the inward half-line is $\Phi_{\mathrm N}(a)$ and its logarithmic derivative is the inverse Mills ratio $\lambda = \phi_{\mathrm N}/\Phi_{\mathrm N}$. Everything else in the paper is that computation carried out on a manifold with corners.
...
The figure script for @fig-mills is [`boundary-layer-figure.py`](boundary-layer-figure.py) in this post's directory; it uses only `numpy`, `scipy` and `matplotlib`.
...
The score is the denoising displacement; the log-Hessian is the posterior covariance and the local linearization of the denoiser.
...
**Generative modeling.** For the heat-regularized law, the population target of denoising score matching at noise level $\sigma$ is exactly $\boldsymbol s_\sigma$ [@hyvarinen2005scorematching; @vincent2011connection; @ho2020ddpm; @song2021scorebased; @karras2022edm], and the canonical flow-matching velocities along the heat path are the fields above [@lipman2023flowmatching; @lipman2024flowmatchingguide]. High-order samplers additionally differentiate them, in $y$ or in $\sigma$ [@lu2022dpmsolver].
...
$$
p_\sigma(\sigma a)
=\underbrace{\rho(0)\,\Phi_{\mathrm N}(a)}_{\textstyle \mathsf C_0}
\;+\;\sigma\,\underbrace{\rho'(0)\bigl(a\,\Phi_{\mathrm N}(a)+\phi_{\mathrm N}(a)\bigr)}_{\textstyle \mathsf C_1}
\;+\;O(\sigma^2),
$$
...
A sanity check: as $a\to+\infty$ the boundary is far away in units of $\sigma$, $\lambda$ and $\lambda'$ vanish, and the score tends to $\beta=(\log\rho)'(0)$, the ordinary interior score. The expansion interpolates between the interior regime and the edge regime, uniformly in $a$.
...
| $\sigma$ | $p_\sigma$, 1 term | $p_\sigma$, 2 terms | $\boldsymbol s_\sigma$, 1 term | $\boldsymbol s_\sigma$, 2 terms | $\mathbf H_\sigma$, 1 term | $\mathbf H_\sigma$, 2 terms |
|---:|---:|---:|---:|---:|---:|---:|
...
The last row is the ratio between consecutive errors: $2$ for the one-term expansions and $4$ for the two-term ones, confirming the $O(\sigma)$ and $O(\sigma^2)$ rates. The density column is more than that: because $\rho$ is affine and the support is flat, all terms beyond $\mathsf C_1$ vanish identically and the two-term expansion is exact up to the exponentially small far-endpoint contribution — which is why that column sits at machine precision rather than at $O(\sigma^2)$.
...
A manifold with corners is stratified: every point has a neighborhood modeled on $\mathbb H^m_c$ for exactly one $c$, and the set of points with a given $c$ is the codimension-$c$ **stratum** $\mathcal S$. For a square, $c=0$ is the open face, $c=1$ the four open edges, $c=2$ the four vertices.
...
with $\pi$ the orthogonal projection onto $\mathcal S$ and $\mathcal K\subset\mathcal S$ compact. This is the only nontrivial regime. At distance $\gg\sigma$ from the singular set one is effectively in the interior and classical expansions apply; at distance $\ll\sigma$ one is at $a\approx 0$, which the layer already contains. The value of stating the results uniformly over $\|a\|\le A$ is precisely that they remain valid *down to the support itself*, where the score is largest and where the expansions that require $\|y-\pi(y)\|\gg\sigma$ have nothing to say.
...
a product of three factors: the density on the support, the volume Jacobian of the chart, and the Gaussian mass of the linearized inward cone seen from $a$. It is the $m$-dimensional version of $\rho(0)\Phi_{\mathrm N}(a)$.

::: {.callout-note appearance="simple"}
## First-order expansion [@brosse2026boundary, Thm. 3]

Under the standing assumption with $r\ge 1$, there is $\sigma_0>0$ such that, uniformly over the layer $\mathcal Y_{A,\mathcal K,\sigma_0}$, with $x=\pi(y)$ and $a=a(y,\sigma)$,
...
The prefactor $\sigma^{-k}$ is bookkeeping, not geometry: $q$ puts finite mass on an $m$-dimensional set and smoothing spreads it across a tube of radius $\sim\sigma$ in the $k$ normal directions, so the ambient density there is $\sim\sigma^{-k}$. It carries no information about boundaries. All the geometry is in $\mathsf C_0$, and it enters through two channels which the coordinates keep separate: the dependence on $a_{\mathcal C}$ records the boundary and corner structure, the dependence on $a_{\mathcal N}$ records the codimension.

The second-order statement [@brosse2026boundary, Thm. 4] adds a coefficient $\mathsf C_1(a,x)$ with $r\ge 2$, so that $p_\sigma=\sigma^{-k}(2\pi)^{-d/2}[\mathsf C_0+\sigma\mathsf C_1+O(\sigma^2)]$ and $\mathsf L_1=\mathsf C_1/\mathsf C_0$. It collects the two ways in which the true integral departs from its linearized cone model at order $\sigma$: the chart is not its own linearization, and the amplitude $\rho\circ\Phi_x\cdot J_x$ is not constant. In the one-dimensional example of @sec-mills, the first source vanishes (the chart is linear) and the second gives $\mathsf C_1=\rho'(0)(a\Phi_{\mathrm N}+\phi_{\mathrm N})$.
...
::: {.callout-note appearance="simple"}
## Expansions of the score and its derivatives [@brosse2026boundary, Thms. 5 and 6]

Uniformly over the layer,
...
Here $\dot{\mathsf S}_0=2\mathbf N(x)a$, so $\dot{\boldsymbol s}_\sigma$ is $O(\sigma^{-2})$ in layer coordinates, where $a$ is bounded. Rewriting $\mathbf N(x)a=(y-\pi(y))/\sigma$ turns this into $O(\sigma^{-3})$ at a genuinely fixed observation point $y$, since such a $y$ has $\|a\|$ growing like $\sigma^{-1}$ as the noise shrinks.

## A smooth boundary: the Mills ratio again

Let $q$ be uniform on the ellipsoid $\mathcal E_{\mathbf A}=\{z:z^\top\mathbf A z\le 1\}$, with constant density $\rho_{\mathbf A}=1/\operatorname{vol}_d(\mathcal E_{\mathbf A})$, so $m=d$, $c=1$, $k=0$. Parametrizing the layer by $y=x-\sigma a\,\mathbf A x/\|\mathbf A x\|$ with $a>0$ pointing inward,
...
The most instructive case is not spelled out in the paper but follows from $\mathsf C_0$ in two lines. Suppose the corner is right-angled in the chart, meaning $\mathbf L(x)=I_m$. Then $\Psi$ separates and $\mathbb H^m_c=\mathbb R^{m-c}\times[0,\infty)^c$ is a product, so
...
$$
\operatorname{Cov}\bigl(X\mid X+\sigma Z=y_\sigma(a,x)\bigr)=\sigma^2\bigl(I_d+\mathsf H_0(a,x,\sigma)\bigr)+O(\sigma^3),
$$

with the leading matrix determined by the tangent cone.

**Lipschitz-in-noise cannot hold.** Some DDIM-type analyses assume regular dependence of the score on the noise level in order to control deterministic-sampler discretization error [@chen2023restoration; @yu2025wasserstein]. The expansion of $\dot{\boldsymbol s}_\sigma$ says this cannot hold uniformly down to $\sigma=0$ for genuinely lower-dimensional data: in the boundary layer, $\partial_\sigma\boldsymbol s_\sigma$ is of order $\sigma^{-2}$ at fixed $y$. Any Lipschitz-in-noise constant must blow up as the terminal noise level goes to zero, and it does so fastest near lower-dimensional supports, boundaries and corners. This gives a geometric account of the low-noise Lipschitz singularities reported empirically [@yang2024lipschitz]. It is a statement about the *population* score; it says the target these analyses assume regular is not, not that a particular trained network misbehaves.

**Inverse problems.** Second-order Tweedie methods express posterior covariance through $\mathbf H_\sigma$ [@boys2024tmpd]. Near a singular support that covariance is strongly anisotropic — tangent, normal and cone directions scale with different powers of $\sigma$ — which is worth accounting for when a diffusion prior is used on constrained or intrinsically low-dimensional data.
