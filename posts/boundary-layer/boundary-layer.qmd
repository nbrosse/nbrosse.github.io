...
@fig-mills and the convergence table below it are produced by `code/boundary_layer_figure.py` in the paper's repository: one-dimensional quadrature for the exact quantities, closed forms for the predicted profiles, and nothing beyond `numpy`, `scipy` and `matplotlib`.
