...
# Error Analysis: Why the Neural Network Struggles to Learn the Marginal Velocity Field

As observed in @fig-article-fig-3, the learned velocity field $u^\theta_t$ exhibits two distinct error spikes when approximating the marginal velocity field $u_t$: one near $t \approx 0.1$ and another near $t \approx 1$. These spikes have fundamentally different origins, which we now explain using the framework developed above.

## Apparent Paradox: Blow-Up vs. Bounded Targets

Before analyzing the error spikes, we address an apparent paradox. The CFM loss trains the neural network with targets $X_1 - X_0$, which are **constant across time** and bounded. Yet the marginal velocity field formula:
$$
u_t(x) = \sum_{i=1}^n \lambda_i(x,t) \frac{x^{(i)} - x}{1-t}
$$
contains a $\frac{1}{1-t}$ factor that blows up as $t \to 1$. How can a neural network match this?

### Resolution: Cancellation at the Interpolant

The key insight is that we evaluate these fields at the **interpolant** $X_t = (1-t)X_0 + tX_1$, not at arbitrary points. At the interpolant, the blow-up is cancelled.
...
$$
u_t(X_t) \approx \lambda_1 \cdot (X_1 - X_0) \approx X_1 - X_0,
$$
which is bounded.

### What About the Non-Collapsed Regime?

Before collapse (small $t$), for indices $i \neq 1$:
$$
x^{(i)} - X_t = x^{(i)} - X_1 + (1-t)(X_1 - X_0),
$$
so:
$$
\frac{x^{(i)} - X_t}{1-t} = \frac{x^{(i)} - X_1}{1-t} + (X_1 - X_0).
$$

The first term **does** blow up as $t \to 1$. However, this is controlled because $\lambda_i \to 0$ exponentially fast for $i \neq 1$ due to the softmax collapse. The product $\lambda_i \cdot \frac{x^{(i)} - X_1}{1-t}$ remains bounded.

### The Real Difficulty: Discontinuity, Not Unboundedness

The paradox resolves: the neural network targets are bounded, and so is the optimal prediction $u_t(X_t)$ at typical interpolant points. The difficulty for neural networks is **not** unbounded values, but rather **sharp spatial discontinuities**:

| Quantity | Behavior | Neural network challenge |
|----------|----------|-------------------------|
| Targets $X_1 - X_0$ | Bounded, constant in $t$ | Easy |
| $u_t(X_t)$ at interpolant | Bounded (cancellation) | OK |
| $u_t(x)$ at Voronoi boundaries | Sharp discontinuities | **Hard** |
| $u_t(x)$ at $t \approx t_C$ | Rapid variation | **Hard** |

We now examine these two sources of difficulty in detail.

## Errors Near $t \approx 1$: The Nearest-Neighbor Regime

Recall the marginal velocity field:
$$
u_t(x) = \sum_{i=1}^n \lambda_i(x,t) \frac{x^{(i)} - x}{1-t}
$$

As $t \to 1$, the softmax temperature $(1-t)^2 \to 0$. This drives the weights $\lambda_i(x,t)$ toward a **one-hot distribution** concentrated on the nearest training point:
$$
\lambda_i(x,t) \xrightarrow{t \to 1} \mathbf{1}\left[i = \arg\min_j \|x - x^{(j)}\|\right].
$$

In this regime, the marginal velocity field approaches:
$$
u_t(x) \approx \frac{x^{(i^*)} - x}{1-t}, \quad \text{where } i^* = \arg\min_j \|x - x^{(j)}\|.
$$

This is a **piecewise-constant vector field** that switches abruptly across **Voronoi cell boundaries** in $\mathbb{R}^d$. Such discontinuous fields are notoriously difficult to approximate with smooth neural networks, explaining the error spike near $t \approx 1$.

## Errors Near $t \approx 0.1$: The Collapse Transition Regime

The error spike near $t \approx 0.1$ is more subtle and directly connected to the REM analysis above.

### Low Information Content at Early Times

Consider the conditional distribution of the interpolant:
$$
x_t \mid (x_1 = x^{(i)}) \sim \mathcal{N}(t x^{(i)}, (1-t)^2 I_d).
$$

For small $t$, the means $tx^{(i)}$ are small compared to the noise standard deviation $(1-t)\sqrt{d}$. The conditional distributions for different training points overlap heavily, making it difficult to infer which $x^{(i)}$ generated a given $x_t$. Consequently, the posterior weights $\lambda_i(x_t, t)$ are **spread across many indices**.

### Sharp Phase Transition at the Collapse Time

The REM analysis reveals that the energy gaps scale as:
$$
E_j - E_1 \approx \frac{d \sigma^2 t^2}{(1-t)^2}.
$$

This creates a **sharp phase transition** around $t_C \approx 0.34$ (for CIFAR-10): for $t < t_C$, the softmax mass is distributed across multiple training points; for $t > t_C$, the planted index dominates with $\lambda_1 \approx 1$.

### Why the Transition Creates Approximation Difficulty

At the collapse transition:

1. **Rapid change in field character:** The velocity field $u_t(x)$ transitions from an averaged field (mixture of many directions) to a nearly-conditional field (pointing toward a single $x^{(i)}$).

2. **High sensitivity:** The winner-takes-all map $x \mapsto \arg\max_i \lambda_i(x,t)$ becomes extremely sensitive to small perturbations in $x$. Near the transition, small changes in $x$ can cause different training points to dominate.

3. **Non-smooth behavior:** The rapid variation of $\lambda_i(x,t)$ as a function of both $x$ and $t$ near $t_C$ creates a vector field that varies sharply over small spatial and temporal scales.

These features make the velocity field near the collapse transition particularly challenging for neural networks to learn, explaining the observed error spike around $t \approx 0.1$ in @fig-article-fig-3.
