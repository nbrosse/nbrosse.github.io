...
$$w_{cropped} = w[w_{min} \leq w \leq w_{max}]$$

$$s_{cropped} = s[w_{min} \leq w \leq w_{max}]$$
...
1. **Cropping** (optional) - Focus on region of interest, reduce data size
...
- **Despiking before smoothing:** Spikes would be smoothed and spread by filters, making them harder to detect. Smoothing after despiking removes any artifacts from spike replacement.

- **Smoothing before baseline:** Noise can confuse baseline fitting algorithms. Smoothing stabilizes baseline estimation, as baseline algorithms assume a relatively smooth underlying signal.

- **Baseline before normalization:** The baseline contributes to normalization metrics (like area under curve) but isn't signal. We must remove the baseline first to normalize only the peaks.

- **Normalization last:** This operates on the final preprocessed spectrum, preserving all previous corrections while only scaling the data.
