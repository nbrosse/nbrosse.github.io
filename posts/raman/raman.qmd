...
The output from the spectrometer is a Raman spectrum, see for example the spectrum in @fig-raman-spectrum-example.
...
We first present a notebook that contains an exploration of the dataset and some preprocessing algorithms. The preprocessing algorithms are presented and explained in @sec-preprocessing-raman.
...
2. **Stage 2: $X$-scores to $Y$-scores (latent regression)** models the relationship between latent scores as $U \approx TG$, where $G \in \mathbb{R}^{K \times K}$ is obtained via least squares:
$$
G = (T^\top T)^{-1}T^\top U
$$
3. **Stage 3: $Y$-scores to $Y$ (reconstruction)** uses the decomposition $Y \approx UQ^\top$ to get $\hat{Y} = UQ^\top$. Combining all stages:
$$
