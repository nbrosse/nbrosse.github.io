...
## A Note on PowerPoint Slides and Code Generation

While this post focuses primarily on Google Slides, Quarto/Reveal.js, and HTML/CSS, it's worth mentioning that Microsoft PowerPoint also supports programmatic slide creation through [Office Scripts](https://learn.microsoft.com/en-us/office/dev/scripts/) (TypeScript for PowerPoint on the web) and [VBA macros](https://learn.microsoft.com/en-us/office/vba/api/overview/powerpoint) (for the desktop app). In theory, you can use code to automate the creation and editing of PowerPoint presentations, much like you can with the Google Slides API.

**How does it work?**  
- With Office Scripts, you write TypeScript code directly in PowerPoint for the web to automate repetitive tasks, generate slides, or manipulate content.
- For more advanced automation, you can use the [Microsoft Graph API](https://learn.microsoft.com/en-us/graph/api/resources/powerpoint-api-overview?view=graph-rest-beta) to interact with PowerPoint files programmatically from external applications.

**Caveat:**  
I did not test LLM-powered PowerPoint code generation for this post. However, based on the available APIs and scripting environments, I expect the experience to be very similar to Google Slides:  
- The LLM would generate TypeScript (Office Scripts) or Graph API calls.
- The process would involve multiple steps: writing code, running it in PowerPoint, and troubleshooting any issues.
- Like Google Slides, the code is often verbose and less readable than Markdown or HTML, and the workflow is less direct than with code-based formats like Quarto/Reveal.js.

**Summary:**  
If you are in a Microsoft 365 environment, you can use LLMs to help generate PowerPoint automation scripts. But expect a similar level of complexity and friction as with Google Slides API code generation.
...
I prompt the LLM to produce the slides in several formats, including Quarto/Reveal.js Markdown, pure HTML/CSS, and Google Slides API code. This allows me to compare how well the LLM handles each format in terms of fidelity, ease of generation, and editability.
...
```{=html}
<iframe src="https://raw.githubusercontent.com/nbrosse/llm-slide-deck/refs/heads/dev/raw/presentation.html" width="100%" height="600px" style="border: 1px solid #ccc;">
    <p>Your browser doesn't support HTML viewing. <a href="https://raw.githubusercontent.com/nbrosse/llm-slide-deck/refs/heads/dev/raw/presentation.html" target="_blank">Click here to download the presentation (HTML)</a></p>
</iframe>
```
...
Of course! This is an excellent question for developers and automation experts. Creating presentations programmatically is a powerful way to automate reporting, generate dynamic content, and integrate data from various sources.

Here is an exhaustive breakdown of the different ways to create Google Slides and PowerPoint slides using only code.

### Summary Table

| Method                                      | Platform         | Primary Use Case                                    | Complexity  | Key Technology                     |
| ------------------------------------------- | ---------------- | --------------------------------------------------- | ----------- | ---------------------------------- |
| **Official APIs**                           |                  |                                                     |             |                                    |
| Google Slides API                           | Google Slides    | Full-control, cloud-native generation & modification | High        | REST, OAuth 2.0, JSON              |
| Microsoft Graph API                         | PowerPoint       | Cloud-native management & basic slide creation      | High        | REST, OAuth 2.0, JSON              |
| **File Format SDKs & Libraries**            |                  |                                                     |             |                                    |
| Office Open XML (OOXML) SDK                 | PowerPoint       | Server-side, deep manipulation of `.pptx` files   | Very High   | .NET (C#), XML                     |
| 3rd Party Libraries (e.g., `python-pptx`)   | PowerPoint       | Simplified, server-side `.pptx` generation        | Medium      | Python, Java, JS, etc.             |
| **Integrated Scripting Environments**       |                  |                                                     |             |                                    |
| Google Apps Script                          | Google Slides    | Simple automation within the Google ecosystem       | Low         | JavaScript                         |
| VBA / Office Add-ins                        | PowerPoint       | In-app automation, requires Office to run         | Medium      | VBA / JavaScript, HTML, CSS        |
| **Markup & Conversion Tools**               | Both             | Fast, content-first, version-controllable slides    | Low         | Markdown, Pandoc, Marp CLI         |


# Building Slide Decks with Microsoft PowerPoint {#sec-microsoft-powerpoint}

Creating PowerPoint files is more diverse because `.pptx` is an open, well-documented, XML-based file format (Office Open XML or OOXML). It's essentially a ZIP file containing XML and other resource files.
