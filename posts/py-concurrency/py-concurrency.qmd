...
::: {.callout-important}
This blog post is adapted from the [Real Python - Python Concurrency](https://realpython.com/python-concurrency/) and [Real Python - Async IO in Python](https://realpython.com/async-io-python/) tutorials. While it summarizes the original content with some modifications and additions, proper attribution is given to Real Python, who retain the copyright of the original material and images. The content is shared for educational purposes.
:::

::: {.callout-note}
All the code snippets in this blog post are regrouped inside the companion GitHub repository [python-concurrency](https://github.com/nbrosse/python-concurrency).
::: 
...
For I/O-bound programs, see @sec-io-bound for detailed examples and timing comparisons. 
For CPU-bound programs, refer to the [Real Python - Python Concurrency](https://realpython.com/python-concurrency/) tutorial which covers CPU-intensive tasks in depth.
...
See @sec-asyncio for a deep dive into asynchronous I/O with AsyncIOs
...
``` {#lst-countasync .python lst-cap="countasync.py"}
import asyncio
import time
...
```{#lst-countsync .python lst-cap="countsync.py"}
import time
...
The typical pattern looks like this and is taken from [llama-index `async_utils.py`](https://github.com/run-llama/llama_index/blob/main/llama-index-core/llama_index/core/async_utils.py).

```{#lst-asyncio-run .python lst-cap="asyncio_run"}
...
``` {#lst-asyncq .python lst-cap="asyncq.py"}
import asyncio
...
This section presents three utility functions from the [llama-index library](https://github.com/run-llama/llama_index/blob/main/llama-index-core/llama_index/core/async_utils.py) that help manage asynchronous tasks in different ways:

1. A function to execute a list of async tasks (with optional progress bar)
2. A function to run tasks in batches to avoid memory issues
3. A function to run tasks with a limited number of workers
...
``` {#lst-run-async-tasks .python lst-cap="llamaindex_async_utils.py"}
def run_async_tasks(
...
``` {#lst-batch-gather .python lst-cap="llamaindex_async_utils.py"}
...
``` {#lst-run_jobs .python lst-cap="llamaindex_async_utils.py"}
async def run_jobs(
